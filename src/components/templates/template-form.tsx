...
export const TemplateForm = (props: Props) => {
  ...
  const { mutate: deleteTemplate } = useDelete<ITemplate>();

  // Get id from route params
  const id = props?.id || window.location.pathname.split("/")[2];
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
        {items.map((item, index) => {
          ...
          return <div key={index}>{`${itemName}: ${item.quantity} ${item.unit}`}</div>;
        })}
      </div>
    );
  };
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
        {fertilizers.map((fertilizer, index) => {
          ...
          return (
            <div key={index}>{`${fertilizerName}: ${fertilizer.quantity} ${fertilizer.unit}`}</div>
          );
        })}
      </div>
    );
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
        {pesticides.map((pesticide, index) => {
          ...
          return (
            <div key={index}>{`${pesticideName}: ${pesticide.quantity} ${pesticide.unit}`}</div>
          );
        })}
      </div>
    );
